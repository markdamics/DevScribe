...
fn toolchain_row(label: &'static str, ok: bool, p: Palette) -> Element<'static, Message> {
    row![
        widgets::dot(color(if ok { p.status_ok } else { p.status_idle }), 6.0),
        text(label)
...
fn left_rail(p: Palette) -> Element<'static, Message> {
    ...
    // larger wordmark.
    let mark = canvas(LogoMark { left: color(p.accent), right: color(p.accent_2) })
    ...
    container(column![top, widgets::hline(color(p.line_neutral)), caption].spacing(16.0).padding([0.0, 0.0]))
...
fn recent_list(state: &State, p: Palette) -> Element<'static, Message> {
    ...
    let header = container(header).width(Length::Fill).style(move |_theme| container::Style {
        background: Some(color(p.bg_panel_2).into()),
        ..container::Style::default()
    });
...
fn action_button(glyph: &'static str, label: &'static str, message: Message, p: Palette) -> Element<'static, Message> {
    button(
        row![
            text(glyph).font(fonts::mono(Weight::Bold)).size(crate::text_scale::px(12.0)),
...
fn loading_overlay(loading: &LoadingProject, p: Palette) -> Element<'static, Message> {
    let panel = container(
        column![
            ...
            text(loading.name.clone())
                ...
                .size(crate::text_scale::px(20.0))
                .color(color(p.text_primary)),
            text(state::shorten_home(&loading.path))
                .font(fonts::mono(Weight::Light))
                .size(crate::text_scale::px(11.0))
                .color(color(p.text_muted)),
            row![progress_segment(45, color(p.accent)), progress_segment(55, color(p.bg_inset))].width(Length::Fill),
    ...
    container(panel)
        ...
        .height(Length::Fill)
        .center(Length::Fill)
        .style(move |_theme| container::Style { background: Some(color(p.bg_scrim).into()), ..container::Style::default() })
        .into()
}
